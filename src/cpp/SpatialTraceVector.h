/*************************************************************************************
    ...
    Source file: SpatialTraceVector.h
...
public:
  ...
  deviceVector<Integer> TMAP;
  deviceVector<Integer> XYZMAP;

  SpatialTraceVector() {};
  ~SpatialTraceVector() { Deallocate(); };
  ...
  Integer *getTmapPointer() {
    buildIndexMap();
    return (Integer *)&TMAP[0];
  }

  Integer *getXYZmapPointer() {
    buildIndexMap();
    return (Integer *)&XYZMAP[0];
  }

  void buildIndexMap() {
    /* Builds mapping from Grid to BLAS indexing, (is, os) -> (t, xyz)
     */
    if (TMAP.size() != 0) {
      return;
    }
    ...
    XYZMAP.resize(lsites);
    TMAP.resize(lsites);
    Integer *xyzMap_p, *tMap_p;
    tMap_p = &TMAP[0];
    xyzMap_p = &XYZMAP[0];

    accelerator_for(os, osites, Nsimd, {
      // Build map from (is, os) to (l_t,l_xyz)

      int lane = acceleratorSIMTlane(Nsimd);
      int idx = lane * osites + os;
      Coordinate lcoor(nd, 0);
      Coordinate icoor(nd);
      Coordinate ocoor(nd);
  ...
  void Trace(std::vector<result_object> &trace_gdata) {
    ...
    tracePush("GridBLAS Init");
    GridBLAS BLAS;
    tracePop("GridBLAS Init");
    ...
    std::cout << GridLogPerformance << " SpatialTraceVector t_import  "
              << t_import << "us" << std::endl;
    std::cout << GridLogPerformance << " SpatialTraceVector t_export  "
              << t_export << "us" << std::endl;
    std::cout << GridLogPerformance << " SpatialTraceVector t_gemm    "
              << t_gemm << "us" << std::endl;
    std::cout << GridLogPerformance << " SpatialTraceVector t_reduce  "
